...
export default function DashBoard() {
    ...
    function Ordenation(paramOrdenation: string) {
        if(paramOrdenation=== 'older')
            return setFilters(dataBase.sort((b, a) => { return a.experience - b.experience }));
        else if(paramOrdenation === 'newer')
            return setFilters(dataBase.sort((a, b) => { return b.experience - a.experience }));
    }
    ...
    return (
        <div className={styles.container}>
            ...
            <FormGroup sx={{ display: 'flex', flexDirection: 'row' }}>
                ...
                <FormControlLabel onClick={()=>Ordenation('older')} control={<Checkbox icon={<StarBorder sx={{ color: 'black' }} />} checkedIcon={<Star />} />} label="Mais antigos" />
...
<Container sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', width: '100%', backgroundColor: 'indigo', borderRadius: '10px', padding: '10px'}}>
            <Table sx= {tableStyles} >
                ...
                <TableBody>
                    {filters.sort()?.map((item, index) =>(<TableRow key={index}>
