...
export default function DashBoard() {
    ...
    function Ordenation(orderType: string, data: [{userId: number, name: string, birthdate: string, occupation: string, experience: number  }])
    {
            const filter: [{ userId: number, name: string, birthdate: string, occupation: string, experience: number }]
                = dataBase.sort((a, b) => b.experience - a.experience)
        setFilters(filter);  
        }
        else if (orderType === 'asc') {
            const filter: [{ userId: number, name: string, birthdate: string, occupation: string, experience: number }]
                = dataBase.sort((a, b) => a.experience - b.experience)
            setFilters(filter);  
        }
                ...
                <FormControlLabel name='filter' onClick={ActivateFilter} control={<Checkbox icon={<StarBorder sx={{ color: 'black' }} />} checkedIcon={<Star />} />} label="Filtrar" />
                    ...
                    {open ? <FormGroup sx={{ display: 'flex', flexDirection: 'row' }}>
                        <RadioGroup sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'row'  }} name="radio-buttons-group">
                            <Radio onClick={() =>Ordenation('asc',dataBase)} icon={<StarBorder/>}   checkedIcon={<Star />} sx={{ color: 'black' }} value="Older" name="Older" /> Mais antigos
                            <Radio icon={<StarBorder/>}   checkedIcon={<Star />} sx={{ color: 'black' }} value="Newer" name="Newer" /> Mais recentes
                            </RadioGroup>
